(function(){
    var shh = {
        ...
        grepNodes : function(searchText, bodyText, frameId) {
          var matchedNodes = [];
          var regXSearch;
        ...
        setUpSecretSocialButton : function(discardNode) { 
          if (!shh.nodeExistsWith(/Go\sPrivate/, window.frames["canvas_frame"].contentDocument.body.innerHTML, "canvas_frame") && !shh.haltButtonCreation) {
            shh.haltButtonCreation = true;
            setTimeout(function(){ shh.haltButtonCreation = false;}, 700);
            discardNode = (discardNode) ? discardNode : shh.grepNodes(/Discard$/g, window.frames['canvas_frame'].contentDocument.body.innerHTML, 'canvas_frame')[0];
            if (!discardNode) {
              return null;
            }
            var secretSocialButton = discardNode.cloneNode();
            secretSocialButton.id = 'ss_button';
            discardNode.parentNode.insertBefore(secretSocialButton, discardNode.parentNode.children[3]);
            shh.secretSocialLink = document.createElement('a');
        ...
        getSubjectText : function(encode) {
          var subjectNode = window.frames['canvas_frame'].contentDocument.getElementsByName("subject")[0];
        ...
        getRecipientsJSON : function() {
          var toNode = window.frames['canvas_frame'].contentDocument.getElementsByName("to")[0];
        ...
        setUpReplyEvents: function() {
          var aReplyNodes = shh.grepNodes(/Reply$/g, window.frames["canvas_frame"].contentDocument.body.innerHTML, "canvas_frame");
          var aReplyToAllNodes = shh.grepNodes(/Reply\sto\sall/g, window.frames["canvas_frame"].contentDocument.body.innerHTML, "canvas_frame");                    
        ...
        init : function() {
            ...
            var composeNode = shh.grepNodes(/Compose mail/, window.frames["canvas_frame"].contentDocument.body.innerHTML, "canvas_frame")[0];            
